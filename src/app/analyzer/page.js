...
//       const res = await fetch("https://krishugsg-production.up.railway.app/analyze", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({
//           username: username.replace("@", "").trim()
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch(
        "https://krishugsg-production.up.railway.app/analyze",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!res.ok) {
        setError("Unable to fetch tweets.")
        setLoading(false)
        return
      }
      ...
      >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 rounded-xl bg-black/30 border border-white/10"
          />
        </div>
        ...
        <button
          onClick={handleAnalyze}
          disabled={loading}
          className="mt-5 w-full bg-pink-500 hover:bg-pink-600 py-3 rounded-xl font-semibold"
        >
          {loading ? "Fetching Tweets..." : "Fetch Tweets"}
        </button>
          ...
          <div className="mt-6 space-y-4 max-h-[420px] overflow-y-auto pr-2">
            {tweets.map((tweet) => (
              <div
                ...
                className="bg-white/5 border border-white/10 rounded-xl p-4"
              >
                <p className="mb-2 text-purple-200">{tweet.text}</p>
                ...
                {tweet.url && (
                  <a
                    href={tweet.url}
                    target="_blank"
                    className="text-pink-400 text-xs underline mt-2 inline-block"
                  >
                    View on X
                  </a>
                )}
        ...
        <button
          onClick={() => router.push("/")}
          className="block mx-auto mt-6 text-sm text-pink-300"
        >
          ⬅ Go Back
        </button>
      </motion.div>
